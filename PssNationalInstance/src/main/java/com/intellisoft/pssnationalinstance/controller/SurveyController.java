...
public class SurveyController {
     ...
     */
    @PostMapping("/add")
    public ResponseEntity<?> addSurvey(@RequestBody DbSurvey dbSurvey) {
        Results results = surveysService.addSurvey(dbSurvey);
    ...
    public ResponseEntity<?> listAdminSurvey(@PathVariable("creatorId") String creatorId, @RequestParam(value = "status", required = false) String status
        ...
        Results results = surveysService.listAdminSurveys(creatorId, status);
        return formatterClass.getResponse(results);

    }

    @GetMapping(value = "/survey-details/{surveyId}")
    public ResponseEntity<?> surveyDetails(@PathVariable("surveyId") String surveyId, @RequestParam(value = "isRespondents", required = false) String respondents

    ) {
        boolean isRespondents = respondents != null && !respondents.equals("") && !respondents.equals("false");


        Results results = surveysService.getSurveyDetails(surveyId, isRespondents);
        return formatterClass.getResponse(results);
    ...
    public ResponseEntity<?> updateSurvey(@RequestBody DbSurvey dbSurvey, @PathVariable("id") String id) {
        Results results = surveysService.updateSurvey(id, dbSurvey);
        return formatterClass.getResponse(results);
    }
